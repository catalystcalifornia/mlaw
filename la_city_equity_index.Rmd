...
# Domain 4 ----
domain_4<-df_all%>%
  select(geoid,bipoc_pctile,diabetes_pctile,imperv_pctile,healthservice_raw_pctile)%>%
  mutate(healthservice_raw_pctile=healthservice_raw_pctile*-1)%>%
  rowwise%>%
  mutate(longevity_count=sum(!is.na(c_across(where(is.numeric)))),
         longevity_score=sum((c_across(where(is.numeric))))/longevity_count)%>%
  ungroup()
...
## Domain Index {.tabset .tabset-fade}
...
### Indicator maps {.tabset .tabset-fade}
...
# feed in indicator label for mapping popup and legend and group overlay
indicator<-paste0("Hazardous Waste Facilities")
direction<-paste0("higher")
...
# feed in indicator label for mapping popup and legend and group overlay
indicator<-paste0("Pedestrian/Bicyclist Fatalities/Injuries")
direction<-paste0("higher")
...
# feed in indicator label for mapping popup and legend and group overlay
indicator<-paste0("ECE Enrollment Rates")
...
# feed in indicator label for mapping popup and legend and group overlay
indicator<-paste0("Eviction Rates")
direction<-paste0("higher")
